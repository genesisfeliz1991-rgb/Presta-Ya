...
let TASAS = { diario: 0.03, semanal: 0.15, mensual: 0.25 };

function registrar() {
  const email = document.getElementById("email").value;
  const password = document.getElementById("password").value;
  if(!email || !password) return alert("Llena correo y contraseña");
...
function cargarTasas(uid) {
  db.collection("config").doc(uid).get().then((doc) => {
    if (doc.exists) {
      ...
      document.getElementById("tasaSemanal").value = (TASAS.semanal * 100);
...
document.getElementById("formPrestamo").addEventListener("submit", (e) => {
  ...
  db.collection("prestamos").add({ uid: user.uid, nombre, monto, tipo, plazo, ...calculo, fecha: new Date() })
  .then(() => { document.getElementById("formPrestamo").reset(); alert("Préstamo guardado ✅"); })
  .catch(err => alert("Error guardando: " + err.message));
});

function cargarPrestamos(uid) {
  db.collection("prestamos").where("uid", "==", uid).orderBy("fecha", "desc")
  .onSnapshot((snapshot) => {
    let tabla = <thead><tr><th>Cliente</th><th>Monto</th><th>Tipo</th><th>Tiempo</th><th>Tasa</th><th>Cuota</th><th>Interés</th><th>Ganancia</th><th>Total</th></tr></thead><tbody>;
    snapshot.forEach((doc) => {
      const p = doc.data();
      tabla += <tr><td>${p.nombre}</td><td>RD$ ${p.monto}</td><td>${p.tipo}</td><td>${p.plazo}</td><td>${p.tasaUsada}</td><td>RD$ ${p.cuota}</td><td>RD$ ${p.interes}</td><td>RD$ ${p.ganancia}</td><td>RD$ ${p.total}</td></tr>;
    });
    tabla += "</tbody>";
